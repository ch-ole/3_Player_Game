...
fig, axs = plt.subplots(2, 11, figsize=(50, 20))
eps = 1e-10
dfs = {}
model = {}
modelp = {}
...
#For B, C and D
dfs_combined_bcd = {i: pd.concat([dfs[f'{prefix}{i}'] for prefix in ["B", "C", "D"]]) for i in range(2, 11)}
#For E, F and G"]]) for i in range(2, 11)}
dfs_combined_efg = {i: pd.concat([dfs[f'{prefix}{i}'] for prefix in ["E", "F", "G"]]) for i in range(2, 11)}
#Trying to create a for loop to avoid having to brute force the graphs
for j, (i, df) in enumerate(dfs_combined_bcd.items()):
...
for j, (i, df) in enumerate(dfs_combined_efg.items()):
        #Making a "modelnumber" variable to store the prefix and i in to compact the code
        modelnumber = f'{prefix}{i}'
        #Using "get" to more safely retrieve the data from the library. If there is none, it will return "none"
        df = dfs.get(modelnumber)
