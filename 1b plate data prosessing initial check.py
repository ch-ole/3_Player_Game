#Plotting data with medical intervention. 
...
#Importing data, only the 1b plate for now
path = r"C:\Users\chris\OneDrive\Dokumenter\Universitet\Master Project\Python\Data sets\plate_1b.csv"
#We find the path by shift clicking on the file and selecting "copy as pathway"
data = pd.read_csv(path)

#Creating a subset of data, a copy of the data which has this subset of data
...
for prefix in prefixes:
    for i in range(2, 12):  # Can adjust the range according to your needs
...
#Trying to create a table for all the slopes to see how they look
for j, prefix in enumerate(prefixes):
    for i in range (2, 12):
        #Calling the dfs[modelnumber]  df to make it easier to write multiple times
        modelnumber = f'{prefix}{i}'
...
#coef_data.to_excel("Resistant_slopes_1b_plate.xlsx")

plt.show()

# Same but for the parental data
coefp_data = pd.DataFrame(columns=["Position on 96 well plate", "Slope of Parental Culture"])
#I want to skip the B, C and D prefixes since they should have 0% parental
prefixesb = ["E", "F", "G"]
for j, prefix in enumerate(prefixesb):
    for i in range (2, 12):
        #Calling the dfs[modelnumber]  df to make it easier to write multiple times
        modelnumber = f'{prefix}{i}'
        #Using "get" to more safely retrieve the data from the library. If there is none, it will return "none"
...
#ACTIVATE THE LINE BELOW TO CREATE A EXCEL FILE WITH THE DATA
#coef_data.to_excel("Parental_slopes_1b_plate.xlsx")
